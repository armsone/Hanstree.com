...
test("renders the OurButton family calling app and release", async () => {
...
test("renders AIBI as a host-integrated engine release", async () => {
  ...
  assert.match(html, /iManagerAI · iOS · Android/);
  assert.match(html, /독립 소스 공개 · iManagerAI에는 검증된 같은 엔진 포함/);
...
test("renders current app release and TestFlight information", async () => {
  ...
  assert.match(stand, /내부 코드 347640/);
  assert.match(stand, /80b79eb3a9d6dd405d97da2577fb9426e99ba74b949bd9dc6a3d450ddd9ca337/);
  assert.match(stand, /사람별 다중 연결 경로/);
  assert.match(stand, /검은 배경이 생기지 않고 포커스 테두리/);
  assert.match(stand, /202608301000/);
  assert.match(stand, /5fd91e504eff5bb1e87b72248f961c2ace7742444f33c520b50a85b212711f92/);
