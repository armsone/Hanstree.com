...
const productCampaigns = {
  ...
  hanclip: {
    ...
    advantages: [
      ...
      ["03", "AISHOT", "실제 스윙과 임팩트가 만나는 순간을 찾습니다.", "AiShot이 준비부터 스윙까지의 화면 움직임과 기기 내 신체 자세 분석을 타격음과 함께 확인합니다.", "레디 같은 말소리나 주변 타석 소리에 덜 반응하고 실제 샷 순간부터 확인하도록 돕습니다."],
  ...
  ccmb: {
    ...
    description: "Codex·Claude·Gemini·Grok의 사용량과 갱신 시간을 네 칸 링에 모읍니다. Grok이 잔량을 제공하지 않으면 이 Mac의 최근 완료 기록으로 24시간 토큰 여유를 추정해 보여 줍니다.",
    ...
    facts: [["04", "AI 서비스 한 패널"], ["02", "메뉴 · 항상 표시 패널"], ["LOCAL", "기존 CLI 세션 활용"]],
    advantages: [
      ["01", "FOUR RINGS", "네 서비스의 남은 여유를 한눈에.", "Codex·Claude·Gemini·Grok의 사용량과 크레딧 정보를 4열 링으로 모읍니다. Grok Free 계정은 이 Mac 기록으로 24시간 토큰 잔량을 추정하고 ‘추정’ 명찰을 붙입니다.", "서비스를 하나씩 열지 않고 다음 작업에 쓸 도구를 빠르게 가늠하며, 공식 수치와 로컬 추정치를 혼동하지 않습니다."],
      ...
      ["06", "NO BUILT-IN KEYS", "새 API 키를 앱에 넣지 않습니다.", "이미 로그인된 로컬 CLI 세션을 활용하고 Grok 인증 갱신·브라우저 로그인은 공식 Grok CLI에 맡기며 분석·원격 텔레메트리를 보내지 않습니다.", "사용량 확인을 위해 별도의 키 보관과 원격 분석을 추가하지 않습니다."],
    ],
    stories: [
      ...
      ["03", "같은 데이터를 다른 도구로", "신선도와 Grok 24시간 토큰 추정치가 포함된 로컬 JSON을 다른 앱과 Codex 대화에서 읽습니다. CCMB에는 새 API 키를 넣지 않고 기존 CLI 세션만 사용합니다.", "로컬 JSON · 추정 명찰 · 읽기 전용"],
    ],
        ...
        {isPrivacy && <>
          ...
          {app.slug === "nasfinder" && <>
            ...
            <section className="limited-use" lang="en" aria-labelledby="google-limited-use-title">
              ...
              <p>The Google Photos integration on iPhone and iPad uses Google Photos Picker so that users explicitly select the media they want to import, and requests only the https://www.googleapis.com/auth/photospicker.mediaitems.readonly scope. NasFinder does not browse or recreate a user&apos;s complete Google Photos library.</p>
