    ...
    <main className={`info-page theme-${app.theme}`}>
      ...
      <article className="legal-shell shell">
        ...
        {isPrivacy && <>
          ...
          <LegalSection title="TestFlight 테스터 사전 신청 정보"><p>홈페이지의 TestFlight 사전 신청 기능은 Apple TestFlight 외부 테스터 선발 및 공식 초대장 발송을 위해 신청자의 이메일, 희망 앱, 사용 기기 모델, 참여 동기 및 동의 시각을 수집합니다. 남용 방지를 위해 접속 IP 주소를 해시로 바꿔 단시간 신청 횟수 제한에만 사용하며 원본 IP 주소는 저장하지 않습니다. 수집된 정보는 테스터 모집 및 테스트 진행 기간 동안 데이터베이스에 보관되며, 관리자 화면의 삭제 기능을 통해 언제든 직접 영구 파기할 수 있습니다. 테스트 인원은 소수로 제한되어 있어 모든 신청자가 선정되지 않을 수 있으며, 삭제를 원하시는 경우 아래 개인정보 보호책임자에게 요청하시면 즉시 삭제 처리됩니다.</p></LegalSection>
